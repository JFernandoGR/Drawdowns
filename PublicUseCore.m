function [Core LossFun]=PublicUseCore(M,Icore)
% This routine takes as its arguments a directed network of ones and zeros and
% uses a greedy algorithm to calculate the minimum loss function core in
% the paper of Craig and von Peter (2010) "Interbank Tiering and Money
% Center Banks".

% The routine takes as INPUTS two vectors, one required, and one optional.
% The first input argument is a matrix, representing a directed network of interbank exposures.
% Its dimension should be square, n x n, where n is the number of banks in
% the system. Each ij element is the exposure of bank i to bank j.  The matrix should
% consist of ones (if the bank i is exposed to bank j) and zeros otherwise. 
% However a simple clearning routine will replace any non zero number by a one.  
% It also changes all self-exposures (diagonal elements) to zeros since the approach is 
% intended for consolidated entities. 

% The second input is optional and represents an initial guess.  This is an
% nx1 vector of ones for the core and zeros for the periphery.  It should
% be chosen far away from the actual core, so that the greedy algorythm is
% more likely to converge to a global optimum.  If this argument is left
% out, then the program will generate a random core that should converge to
% something close to the global optimum.

% The OUTPUTS consist of two vectors, Core, and LossFun. The first is an
% nx1 vector of ones for the banks that are core banks and zeros for the periphery.  
% The second, LossFun, is a 1x4 matrix that has the error score of the 
% various blocks [CC CP; PC PP] of the loss function.

% The usual disclaimers regarding the use of this code apply. Not for distribution without the authors' consent.
% Ben Craig 2011
...
original.RRCP=sum(1-any(CP,2));             % row-regular: Note that this does not assign 
                                            %penalty yet
original.CRCP=sum(1-any(CP,1));             % col-regular: Punish zero cols (peri not connected)
                                            %Same not as above
% From periphery to core (PC block)         % regular block again requires both:
original.CRPC=sum(1-any(PC,1));             % col-regular 
original.RRPC=sum(1-any(PC,2));             % row-regular 
...
Err=[newerr.CC errCP errPC newerr.PP];      % Unweighted error matrix and score
end